...
export class PedidosComponent implements OnInit {
  ...
  platillosFiltrados: PlatilloConCantidad[] = []; // Array extendido con 'cantidad'
  ...
  eliminarDelCarrito(indice: number): void {
    this.carrito.splice(indice, 1); // Eliminamos el elemento del carrito
  }
  ...
  registrarPedidos(): void {
    ...
    this.carrito = []; // Vaciamos el carrito después de registrar
  }
...
interface PlatilloConCantidad extends Platillo {
  cantidad: number; // Nueva propiedad para manejar la cantidad en el binding
}